...
async function guardAgainstUnsavedChanges() {
  ...
  const flag = isDirty() ? "yes" : "no";
  console.log("Dirty flag is ", flag);
  if (isDirty()) {
    //$("nameInput").value = newMemberMode ? newMemberName : currentMember.name;
    const temp = (currentMember) ? "yes" : "no";
    console.log(`currentMember set? ${temp}`);
    if (currentMember) $("nameInput").value = currentMember.name;
    else $("nameInput").value = newMemberName;
    //newMemberMode = false;
    console.log(`new member is ${newMemberName}, hold is ${hold}`);
...
async function selectComboItem(name) {
  ...
  //if (!newMemberMode) {
    console.log(`guard first received ${name}`);
  ...
  //}
  // 2. Safe to switch
  console.log(`guard switching to ${name}`);
...
export function onNameInputKey(e) {
  if (e.key === "Enter") {
    e.preventDefault();   // stop form submission or default behavior
    e.stopPropagation();
    $("nameInput").blur();   // force blur → triggers your blur handler
  }
}

export function onComboInputBlur() {
  console.log(`just entered ${newMemberName}`);
  //const typed = $("nameInput").value.trim();
  if (!newMemberName) return;
  if (newMemberMode) {
    const match = checkMemberExists(newMemberName);
    if (match) {
      //newMemberMode = false;
      $("nameInput").value = match.name;
      console.log(`Passing ${match.name}`);
      //nameSelected();
      selectComboItem(match.name);
      return;
    }
    hideAnswerSection(newMemberName);
  }
}

function checkMemberExists(name) {
  return jsonData.family.find(
...
/**************************************************************
 * hideAnswerSection
 * Hide or display the answer section depending on name
 **************************************************************/
function hideAnswerSection(mem) {
  const answersSection = $("answersSection");

  isWildcard = mem.toLowerCase().startsWith("wildcard");
  isFollies = mem.toLowerCase() === "follies";

  answersSection.style.display = (isWildcard || isFollies) ? "none" : "block" ;
  ...
  $("storyLabel").textContent = 
    isWildcard ? "Wildcard Main Title:" :
    isFollies ? "Folly Main Title:" :
    "Story:";
  $("questionLabel").textContent = 
    isWildcard ? "Wildcard Subtitle:" :
    isFollies ? "Folly Subtitle:" :
    "Question:";
...
function validateRequiredFields() {
  const isSpecial = isFollies || isWildcard;
  const fields = [
      $("nameInput"),
      $("storyBox"),
      $("questionBox"),

...
async function selectComboItem(name) {
  //if (newMemberMode) return;  //ignore clicks in new member mode
  // 1. Guard against unsaved changes
  //alert("In selectComboItem now.");
  const guard = await guardAgainstUnsavedChanges();
  if (guard == "block") {
    $("comboList").classList.add("hidden");
    //alert("Should have just hidden the list.");
    return;
  }
  //alert("It seems we passed the guard.");
  // 2. Safe to switch
...
export async function onComboInput() {
  ...
  if (guard === "exit") {  // User wants to forget about changes
    ...
    /*$("comboList").classList.remove("hidden");
    console.log("Dropdown visible immediately:", !$("comboList").classList.contains("hidden"));
    setTimeout(() => {
      console.log("Dropdown visible after 50ms:", !$("comboList").classList.contains("hidden"));
    }, 50);*/
  }
  ...
  newMemberName = input;
/**************************************************************************************
 * Here we have a little problem. If newMemberMode is true, we don't get to reset 
 * everything here. I don't want to reset on every extra letter we type in the name
 * after setting newMemberMode. I think there are other places where I still want to
 * know that we're in newMemberMode. Like, if I'm in newMemberMode already and I click
 * on the newMember button, I want to go through the guard and reset. Or, if I'm in 
 * newMemberMode and want to change the name, I should be able to retype it - until I 
 * save it at which point we leave newMemberMode. 
 * New thought. If I'm in newMemberMode, don't respond to the newMember button or the rename -
 * in fact, maybe in newMemberMode we get rid of those buttons along with Add New Anecdote, 
 * Delete Anecdote, and Delete Person.
 * Do I need another variable - newMemberStarted or alreadyCleared?
 * The problem is I want newMemberMode to remain on, but I only want to do this bit once.
 ...
 * These three functions deep track of changes in the fields
